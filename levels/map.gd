...
func _ready():
	...
	for x in region.size.x:
		for y in region.size.y:
			...
			var l1_tile_data: TileData = self.get_cell_tile_data(0, tile_position)
			var l2_tile_data: TileData = self.get_cell_tile_data(1, tile_position)
			var l3_tile_data: TileData = self.get_cell_tile_data(2, tile_position)
			if not l1_tile_data or l1_tile_data.get_custom_data("Impassible"):
